...
export class GameService {
     ...
     */
    dealCardToPlayer(player, card, isInitialDeal = false, isResolvingFlipThree = false) {
        if (!this.gameState)
            return;
        ...
        if (card.type === 'number' && !isInitialDeal) {
            ...
            const hasDuplicate = player.numberCards.some(existingCard => existingCard.type === 'number' && existingCard.value === card.value);
            if (hasDuplicate) {
                ...
                if (hasSecondChanceCard) {
                    ...
                    const usedSecondChanceCardIds = new Set(player.cards
                        .filter(c => {
                        ...
                        .map(c => c.id));
                    const unusedSecondChanceCard = secondChanceCards.find(c => !usedSecondChanceCardIds.has(c.id));
                    if (unusedSecondChanceCard) {
                        ...
                        const remainingUnused = secondChanceCards.filter(c => c.id !== unusedSecondChanceCard.id && !usedSecondChanceCardIds.has(c.id));
