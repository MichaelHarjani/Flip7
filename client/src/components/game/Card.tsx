...
export default function Card({
  ...
  const getCardContent = () => {
    ...
    if (card.type === 'action') {
      const actionConfig: Record<string, { display: string; bgColor: string; borderColor: string; textColor: string }> = {
        freeze: {
          display: '🧊',
          bgColor: 'bg-cyan-300',
    ...
    return { display: '?', bgColor: 'bg-gray-300', borderColor: 'border-gray-600', textColor: 'text-gray-900' };
  };
  ...
  return (
    <div className={`relative ${className} group`} style={{ perspective: 1000 }}>
      ...
      {showTooltip && !isFlipped && (
        <div className="absolute hidden group-hover:block bg-black/90 text-white px-2 py-1 rounded text-xs -top-10 left-1/2 transform -translate-x-1/2 whitespace-nowrap z-50 shadow-xl">
          {getTooltipContent()}
        </div>
      )}
    </div>
